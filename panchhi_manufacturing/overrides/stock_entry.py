...
class MultiVariantStockEntry(StockEntry):
	...
	def check_if_operations_completed(self):
		"""Stock demands a completed Job Card for EVERY Work Order
		operation before a Manufacture entry can post more finished qty.
		A SUBCONTRACTED operation transacts through a Subcontracting Order
		and never gets a Job Card, so it would block the operation's own
		SFG-receipt entry with 'Job Card not found'. Skip subcontracted
		operations here — the mirror of MultiItemJobCard.validate_sequence_id
		— and enforce the check unchanged for in-house operations."""
		...
		for d in prod_order.get("operations"):
			...
			if flt(total_completed_qty, self.precision("fg_completed_qty")) > flt(
				...
				frappe.throw(
					_(
						"Row #{0}: Operation {1} is not completed for {2} qty of finished goods "
						"in Work Order {3}. Please update operation status via Job Card {4}."
					).format(
						d.idx,
